...
func ReceiveMessage(pc sarama.PartitionConsumer, partitionMap *sync.Map, partition int32) {
	...
	var sceneTestResultDataMsgSource = new(kao.SceneTestResultDataMsg)
	...
	for msg := range pc.Messages() {
		...
		case <-timeoutTicker.C:
			if time.Now().UnixMilli()-timeout < 60000 {
				continue
			}
		...
		case <-sendMsgTicker.C:
			if sceneTestResultDataMsg.ToJson() == sceneTestResultDataMsgSource.ToJson() {
				continue
			}
